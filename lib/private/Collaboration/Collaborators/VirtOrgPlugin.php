...
class VirtOrgPlugin implements ISearchPlugin {
	/** @var bool */
	...
	public function __construct(IConfig $config, ICloudFederationProviderManager $cloudFederationProviderManager, ICloudIdManager $cloudIdManager, IGroupManager $groupManager, IUserSession $userSession) {
		try {
	...
	public function searchRemoteGroup($search, $limit, $offset, ISearchResult $searchResult) {
		$result = ['wide' => [], 'exact' => []];
	...
	public function search($search, $limit, $offset, ISearchResult $searchResult) {
		...
		foreach ($groups as $group) {
			...
			$groupBackends = array_map(function($backendName) {
				$backends = $this->groupManager->getBackends();
				foreach($backends as $backend) {
					if ($backend instanceof INamedBackend) {
						if ($backend->getBackendName() == $backendName) {
							return $backend;
						}
					}
				}
			...
			$isVirtOrg = array_reduce($groupBackends, function ($isVirtOrg, $backend) use (&$group) {
				return $isVirtOrg | ($backend instanceof IVirtualOrganisationBackend && $backend->isVirtualOrganisation($group->getGID()));
			}, false);
			...
			if (strtolower($gid) === $lowerSearch || strtolower($group->getDisplayName()) === $lowerSearch) {
				$result['exact'][] = [
					...
					'value' => [
						...
						'shareWith' => $gid,
						'server' => substr($gid, strpos($gid, "#") + 1),
					],
				];
			} else {
			...
			if ($group instanceof IGroup && !$group->hideFromCollaboration() && (!$this->shareWithGroupOnly || in_array($group->getGID(), $userGroups))) {
				$result['exact'][] = [
					...
					'value' => [
						...
						'server' => substr($group->getGID(), strpos($group->getGID(), "#") + 1)
					],
				];
			}
		}
